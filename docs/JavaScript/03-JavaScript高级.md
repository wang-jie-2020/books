...
js不是面向对象设计的，它模拟面向对象的设计思想。

以函数式的声明代替面向对象中类的概念。

以函数原型实现继承，当然和其他语言一样，链的顶端都是object，理解起来差不多，构造函数不是默认继承的，需要手动添加一下。

### 创建对象

#### 简单方式---通过new Object()创建或字面量创建
...
- 构造函数是根据具体的事物抽象出来的抽象模板（可理解成类）
- 实例对象是根据抽象的构造函数模板得到的具体实例对象（可理解为类的实例）
...
Javascript 规定，每一个构造函数都有一个 `prototype` 属性，指向另一个对象（指向一个类型）。
...
<img src="./images/relations.png" alt="">
...
以上代码执行结果在不同浏览器中结果不一致，因为提升声明时可能出现覆盖。
