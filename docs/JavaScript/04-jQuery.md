...
DOM---window.onload，在页面全部加载完毕才触发
...
```javascript
//jQuery中第一种页面加载的事件，和DOM类似，但两个方法都会被调用
...
```javascript
//jQuery中第二种页面加载的事件----比上面的快---页面中的基本的元素加载后就触发
$(document).ready(function () {
    console.log("1");
});
$(document).ready(function () {
...
```javascript
JavaScript的入口函数要等到页面中所有资源（包括图片、文件）加载完成才开始执行。有覆盖问题
...
在以jQuery选择器获取元素时，不再需要循环遍历代码，因为内部已经帮助我们做好了隐式的迭代。
...
- 作用：复制匹配的元素
  ...
  注意：若获取的元素是html已经存在的，那么其通过append方法添加到另一个元素中的时候，类似于剪切
...
attr方法主要是操作元素的自定义属性的,但是也可以操作元素的自带的属性(html中本身就有的属性),但是操作元素的选中的ckeched的时候,不是很合适!
...
```javascript
//val方法也有选中的意思，为select标签中value属性是5的这个option标签选中
$("#s1").val(5);//选中的意思
```
...
//这种方式可以触发该事件,但是,不能触发浏览器的默认的行为，比如focus是光标不定位但已经触发了事件处理
